...
def ask_speed_and_angle():
    ...
    try:
        new_speed = float(eval(input(f"inital speed in [m/s] (enter={speed_default})")))
    except ValueError:
    ...
    try:
        new_angle = float(eval(input(f"angle in degree (enter={angle_default})")))
    except ValueError:
...
def display_parameters():
    ...
    keys = [
        ...
        if "__" not in k and k not in ("angle", "speed", "history")
    ]
    max_length = max([len(key) for key in list(Data.__dict__.keys())])
    max_length2 = max(
        [len(str(value)) for k, value in list(Data.__dict__.items()) if k in keys]
    )
...
def ask_game_parameters():
    ...
    for key in keys:
        ...
        try:
            new_value = float(
                eval(input(f"enter new value for {key}: (enter=accept {value})"))
            )
        except ValueError:
            new_value = value
        if new_value != value:
...
def play(show_xy=False):
    ...
    print(
        "change angle and speed to hit the target as close as possible. (or enter 0 for both values to quit)"
    )
    number = 1
    while True:
        ...
        max_range = abs(
            ...
            * (
                ...
                + (
                    math.sin(math.radians(Data.angle)) ** 2
                    + (2 * Data.y0 * Data.gravity / Data.speed ** 2)
                )
                ** 0.5
            )
        )
        ...
        if abs(distance_to_target) < Data.critical_distance_to_target:
            ...
            print(
                f"Too short! Your shot lands {abs(distance_to_target)} m before the target "
            )
        else:
            print(
                f"Too wide! Your shots lands {abs(distance_to_target)} m behind the target"
            )
        print("try again....")
        number += 1
...
def game():
    ...
    while True:
        ...
        command = eval(input(
            "[c]hange game parameters, [p]lay, play with [d]etails,  [q]uit  (enter=play)"
        ))
