...
class Data:
    ...
    critical_distance_to_target = (
        15  # how close to the target a cannonball must land to count as a hit
    )
...
def calculate_world():
    ...
    Data.__upper_right_y = max(
        Data.cannon_y + 10, Data.target_y + 10, Data.target_x + 10
    )
...
def shoot():
    ...
    best_distance_to_target = (
        (Data.cannon_x - Data.target_x) ** 2 + (Data.cannon_y - Data.target_y) ** 2
    ) ** 0.5
    ...
    Data.history.insert(
        0, text + ", " + break_reason
    )  # newest history line should be on top
    Data.__number += 1
...
def paint_cannon_and_target():
    ...
    plot.circle_cross(
        ...
        Data.cannon_y,
        size=20,
        angle=math.radians(Data.angle - 90),
        legend_label="cannon",
        alpha=0.5,
        name="cannon",
    )
    ...
    plot.circle(
        Data.target_x,
        Data.target_y,
        legend_label="target radius",
        radius=Data.critical_distance_to_target,
        line_color="black",
        line_dash="dotted",
        fill_color=None,
        name="target_radius",
...
paint_cannon_and_target()
# Set up widgets
angle_widget = bokeh.models.Spinner(
    title="angle in [Grad]:", high=180, low=-180, step=0.1, value=Data.angle
)
speed_widget = bokeh.models.Spinner(
    title="speed in [m/s]:",
    value=Data.speed,
    high=10000,
    low=0,
...
gravity_widget = bokeh.models.Spinner(
    title="gravity in [m/s²]:", high=50, low=-50, step=0.1, value=Data.gravity
)
...
cannon_x_widget = bokeh.models.NumericInput(
    title="cannon x in [m]:", value=Data.cannon_x, mode="float"
)
cannon_y_widget = bokeh.models.NumericInput(
    title="cannon y in [m]:", value=Data.cannon_y, mode="float"
)
target_x_widget = bokeh.models.NumericInput(
    title="target x in [m]:", value=Data.target_x, mode="float"
)
target_y_widget = bokeh.models.NumericInput(
    title="target y in [m]:", value=Data.target_y, mode="float"
)
dt_widget = bokeh.models.Spinner(
    title="delta time (dt)  in [s]", value=Data.dt, high=0.99, low=0.01, step=0.01
)
t_max_widget = bokeh.models.NumericInput(
    title="max. number of timesteps", value=Data.t_max, mode="float"
)
crit_distance_widget = bokeh.models.NumericInput(
    title="target radius in [m]", value=Data.critical_distance_to_target, mode="float"
)
...
my_widgets = bokeh.layouts.column(
    angle_widget,
    speed_widget,
    gravity_widget,
