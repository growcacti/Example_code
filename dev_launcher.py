...
if __name__ == "__main__":
    ...
    if os.path.exists("./locale"):
        languages = [
            ...
            if os.path.isdir("./locale/%s" % file)
            and os.path.isfile("./locale/%s/activity.linfo" % file)
        ]
    ...
    output = [
        ...
        (
            ...
            "Autodetect"
            if options.res == (0, 0)
            else str(options.res[0]) + " x " + str(options.res[1]),
        ),
    ...
    if not options.profile:
        try:
            spyral.director.init(
                options.res, fullscreen=options.fullscreen, max_fps=options.fps
            )
            launch()
            spyral.director.run()
