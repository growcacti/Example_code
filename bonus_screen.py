...
class BonusScreen(tools._State):
    ...
    def make_labels(self):
        ...
        self.label1 = StainedGlass(
            (cx, cy - dist),
            prepare.GFX["word-bonus"],
            prepare.GFX["cover-bonus"],
            colors,
            120,
            1500,
        )
    ...
    def select_bonus(self, color, gem_num):
        ...
        ani3 = Animation(width=w * 2, height=h * 2, duration=1000, round_values=True)
    ...
    def make_bonus_icons(self):
        ...
        self.icons = cycle(
            [BonusIcon(center, *combo) for combo in self.grid.jewel_combos]
        )
    ...
    def update(self, dt):
        ...
        self.grid.update(dt)
        if not any((self.grid.animations, self.grid.points_animations, self.leaving)):
            self.make_cover()
            duration = 1000
