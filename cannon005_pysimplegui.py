...
class Data:
    ...
    critical_distance_to_target = (
        1  # how close to the target a cannonball must land to count as a hit
    )
...
def main():
    ...
    layout_buttons = sg.Column(
        vertical_alignment="top",
        layout=[
            [
                sg.Text("speed:", size=(6, 1), justification="right"),
                sg.Input(key="speed", size=(10, 1), default_text=Data.speed),
            ],
            [
                sg.Text("angle:", size=(6, 1), justification="right"),
                sg.Input(key="angle", size=(10, 1), default_text=Data.angle),
            ],
    ...
    layout_settings = sg.Column(
        layout=[
            [
                sg.Text(k + ":", size=(22, 1), justification="right"),
                sg.Input(key=k, size=(14, 1), default_text=v, disabled=True),
            ]
            for k, v in list(Data.__dict__.items())
            if "__" not in k and k not in ("history", "speed", "angle")
        ],
    )
    layout_history = sg.Multiline(
        default_text=Data.history,
        key="history",
        disabled=True,
        autoscroll=True,
        size=(40, 12),
    ...
    layout = [
        [
            sg.Text("play:", size=(20, 1)),
            sg.Text("settings:", size=(45, 1)),
            sg.Text("history:", size=(30, 1)),
        ],
    ...
    calculate_world(
        window["graph"]
    )  # for graph size, change hidden size attributes inside Data
    window.finalize()
...
def shoot(graph_widget):
    ...
    while t < Data.t_max:
        ...
        dx = x - Data.__ball_x
        dy = y - Data.__ball_y
        graph_widget.draw_line(
            point_from=(Data.__ball_x, Data.__ball_y),
            point_to=(x, y),
            color="red",
            width=1,
        )
        ...
        # vector math
        distance_to_target = (
            (Data.__ball_x - Data.target_x) ** 2 + (Data.__ball_y - Data.target_y) ** 2
        ) ** 0.5
        break_reason = None
...
def calculate_world(graph_widget):
    ...
    Data.__upper_right_y = max(
        10, Data.cannon_y * 1, Data.target_x * 2, Data.target_y + 10
    )
    graph_widget.change_coordinates(
        graph_bottom_left=(int(Data.__lower_left_x), int(Data.__lower_left_y)),
...
def make_grid(graph_widget):
    ...
    graph_widget.erase()
    graph_widget.draw_line(
        point_from=(Data.__lower_left_x, 0),
        point_to=(Data.__upper_right_x, 0),
        color="grey",
        width=2.5,
    )
    graph_widget.draw_line(
        point_from=(0, Data.__lower_left_y),
        point_to=(0, Data.__upper_right_y),
        color="grey",
        width=2.5,
    )
    graph_widget.draw_text(
        text="y",
    ...
    for y in range(
        int(Data.__lower_left_y),
        int(Data.__upper_right_y),
        (int(Data.__upper_right_y) - int(Data.__lower_left_y)) // 20,
    ):
        graph_widget.draw_line(
            point_from=(Data.__lower_left_x, y),
    ...
    for x in range(
        int(Data.__lower_left_x),
        int(Data.__upper_right_x),
        (int(Data.__upper_right_x) - int(Data.__lower_left_x)) // 20,
    ):
        graph_widget.draw_line(
            point_from=(x, Data.__lower_left_y),
    ...
    Data.__cannon_id = graph_widget.draw_rectangle(
        top_left=(Data.cannon_x - 1, Data.cannon_y + 1),
        bottom_right=(Data.cannon_x + 1, Data.cannon_y - 1),
        fill_color="blue",
        line_color="green",
        line_width=5,
    )
    Data.__cannon_text_id = graph_widget.draw_text(
        text="             cannon",
        font=("Arial", 8),
        angle=80,
    ...
    Data.__target_id = graph_widget.draw_circle(
        center_location=(Data.target_x, Data.target_y),
        radius=2,
        fill_color="black",
        line_color="red",
        line_width=2,
    )
    Data.__target_text_id = graph_widget.draw_text(
        text="                  target",
        font=("Arial", 8),
        angle=80,
